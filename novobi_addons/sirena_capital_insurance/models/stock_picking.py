...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        ...
        if self.second_is_create_label and self.second_fedex_service_type in ['GROUND_HOME_DELIVERY', 'FEDEX_GROUND'] and self.second_capital_insurance_quote_id:
    ...
    def button_void_label(self):
        ...
        if self.is_void_first_label and fedex_service_type in ['GROUND_HOME_DELIVERY', 'FEDEX_GROUND'] and self.capital_insurance_quote_id:
            ...
            if insurance_result['result'] == 'ERROR' and 'Void is not allowed on unconfirmed quote' not in insurance_result['error_message']:
                log_message = (_("Shipping Option 1<br/>Failed to void UPS Capital Insurance: %s") % (insurance_result['error_message']))
                self.message_post(body=log_message)
            elif insurance_result['result'] == 'SUCCESS' or 'Void is not allowed on unconfirmed quote' in insurance_result.get('error_message', ''):
        ...
        if self.is_void_second_label and second_fedex_service_type in ['GROUND_HOME_DELIVERY', 'FEDEX_GROUND'] and self.second_capital_insurance_quote_id:
            ...
            if second_insurance_result['result'] == 'ERROR' and 'Void is not allowed on unconfirmed quote' not in second_insurance_result['error_message']:
                log_message = (_("Shipping Option 2<br/>Failed to void UPS Capital Insurance: %s") % (second_insurance_result['error_message']))
                self.message_post(body=log_message)
            elif second_insurance_result['result'] == 'SUCCESS' or 'Void is not allowed on unconfirmed quote' in second_insurance_result.get('error_message', ''):
