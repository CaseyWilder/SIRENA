...
class InboundShipmentPlanEpt(models.Model):
    ...
    def create(self, vals):
        ...
        return super(InboundShipmentPlanEpt, self).create(vals)
    ...
    def reset_all_lines(self):
        ...
        for record in result:
            duplicate_lines = self.mapped('shipment_line_ids').filtered( \
                lambda r: r.amazon_product_id.id == record[0])
            qty = 0.0
            quantity_in_case = 0
    ...
    def create_procurements(self, odoo_shipments, job=False):
        ...
        for shipment in odoo_shipments:
            ...
            warehouse = fulfillment_center and fulfillment_center.warehouse_id or ship_plan.instance_id.fba_warehouse_id and ship_plan.instance_id.fba_warehouse_id or ship_plan.instance_id.warehouse_id or False
    ...
    def cancel_entire_inbound_shipment(self, shipment, job):
        """
        This method is used to cancel the entire inbound shipment
        """
        log_line_obj = self.env['common.log.lines.ept']
        ship_plan = shipment.shipment_plan_id
        ...
        if response.get('reason'):
            ...
            log_line_obj.create({
                'message': error_value,
                'model_id': log_line_obj.get_model_id('inbound.shipment.plan.ept'),
                'res_id': self.id,
                'log_book_id': job.id
            })
        else:
    ...
    def create_inbound_shipment_plan(self):
        ...
        if is_are_cases_required:
            zero_qty_in_case_list = self.shipment_line_ids.filtered( \
                lambda r: r.quantity_in_case <= 0)
        ...
        x = 0
        while True:
            shipment_lines = self.mapped('shipment_line_ids').filtered( \
                lambda r: r.odoo_shipment_id is not set and r.id not in already_taken)
