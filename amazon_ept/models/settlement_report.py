...
class SettlementReportEpt(models.Model):
        ...
        for row in settlement_reader:
            ...
            if order_ref in order_name_list and amz_order and row.get('transaction-type') == 'Order':
                ...
                order_statement_lines.filtered(
                        lambda l: l.name == order_line_name and not l.sale_order_id).write( \
    ...
    def reconcile_reimbursement_lines(self, seller, bank_statement, statement_lines, \
        ...
        for line in statement_lines:
            ...
            reimbursement_invoice = reimbursement_invoice_ids.filtered( \
                    lambda l: l.state in ['draft', 'posted'] and not l.payment_state in ['paid', 'in_payment'] and l.invoice_date == date_posted and l.move_type == invoice_type)
            if len(reimbursement_invoice) > 1:
                reimbursement_invoice = reimbursement_invoice.filtered(lambda l: round(l.amount_total,10) == round(line.amount, 10))
            ...
            if not reimbursement_invoice:
                ...
                self.create_amazon_reimbursement_invoice_line(seller, reimbursement_invoice, line.name, line.amount, trans_line)
    ...
    def reconcile_orders(self, statement_lines):
        ...
        for statement_line_id in statement_lines:
            ...
            if len(paid_invoices) > 1:
                paid_invoices = invoices.filtered(lambda l: round(l.amount_total,10) == round(statement_line.amount, 10))
    ...
    def process_settlement_orders(self, bank_statement, settlement_id, orders_list):
        ...
        bank_line_vals = [{'name': "%s/%s" % (order_key[5], order_key[0]), 'payment_ref': settlement_id, 'partner_id':
            order_key[4], 'amount': invoice_total,
                           'statement_id': bank_statement.id, 'date': order_key[2], 'sale_order_id': sale_order_obj.browse(order_key[1]).filtered(
                    lambda l: round(l.amount_total, 10) == round(invoice_total, 10)) and sale_order_obj.browse(order_key[1])[0].id or sale_order_obj.browse(order_key[1]).id if order_key[4] != 'MFN' else sale_order_obj.browse(order_key[1]).id}
    ...
    def check_or_create_invoice_if_not_exist(self, amz_order):
        ...
        for order in amz_order:
            ...
            # Check Order is confirmed and invoice status is remaining to invoice
    ...
    def create_refund_invoices(self, refund_list_item_price, bank_statement):
        ...
        for order_key, product_amount in refund_list_item_price.items():
            ...
            if refund_exist:
                is_refund_line = statement_line_obj.search(
                    [('sale_order_id', '=', order.id), ('refund_invoice_id', 'in', refund_exist.ids),
                     ('statement_id', '=', bank_statement.id), ('is_refund_line', '=', True)])
                refund_exist = False if is_refund_line else refund_exist[0]
            if not refund_exist:
                invoices = order.invoice_ids.filtered( \
                        lambda
                ...
                refund_process = refund_obj.create({
                    'move_ids': [(6, 0, invoices[0].ids)],
                    'reason': 'Refund Process Amazon Settlement Report'
                })
                refund = refund_process.reverse_moves()
                refund_invoices = [refund.get('res_id')] if refund.get('res_id', False) else refund.get('domain')[0].__getitem__(2)
                for ref_inv in refund_invoices:
                    refund_invoice = refund_record.browse(ref_inv)
