...
class SaleOrderLine(models.Model):
                ...
                if not sale_line:
                    ...
                    if available_line:
                        ...
                        if available_line.price_unit == unit_price:
                            updated_qty = float(
                                line_data.get('quantity-shipped')) + available_line.product_uom_qty
                        ...
                        if line_data.get('quantity-shipped'):
                            order_line_vals.update(
                                {'product_uom_qty': float(line_data.get('quantity-shipped'))})
    ...
    def create_sale_order_line_vals_amazon(self, order_line, odoo_product, amazon_order):
        ...
        vals = ({
            ...
            'order_qty': order_line.get('product_uom_qty', False) or order_line.get(
        ...
        order_vals.update({
            ...
            'amazon_shipment_item_id': order_line.get('amazon_shipment_item_id',
                                                      False) or order_line.get(\
    ...
    def create_amazon_order_charges_line(self, amazon_order_line, line_data):
        ...
        if line_data.get('ship-promotion-discount') and float(
                line_data.get('ship-promotion-discount')) < 0:
            ...
            if shipment_discount_product:
                ...
                discount_price = float(line_data.get('ship-promotion-discount')) - ship_tax
                if instance.is_use_percent_tax:
        ...
        """Promotion Discount"""
        if line_data.get('item-promotion-discount') and float(\
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        """
        self.ensure_one()
        if self._context.get('shipment_item_ids'):
