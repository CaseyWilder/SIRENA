...
class StockPicking(models.Model):
        ...
        for picking in pickings:
            ...
            for item in items:
                ...
                move_lines = picking.move_lines.filtered(
                    lambda move_lines: move_lines.product_id.id == odoo_product_id and move_lines.state not in
                                       ('draft', 'done', 'cancel', 'waiting'))

                if not move_lines:
                    move_lines = picking.move_lines.filtered(
                        lambda move_line: move_line.product_id.id == odoo_product_id and move_line.state not in
                                          ('draft', 'done', 'cancel'))
    ...
    def amz_inbound_get_stock_move_ept(picking, odoo_product_id):
        ...
        stock_move = picking.move_lines.filtered( \
            lambda r: r.product_id.id == odoo_product_id.id and r.state not in ( \
                'draft', 'done', 'cancel', 'waiting'))
        if not stock_move:
            stock_move = picking.move_lines.filtered( \
                lambda r: r.product_id.id == odoo_product_id.id and r.state not in ( \
    ...
    def check_qty_difference_and_create_return_picking(self, response, amazon_shipment_id,
        ...
        if return_picking:
            return_picking.action_confirm()
            return_picking.action_assign()
            stock_immediate_transfer_obj.with_context(default_pick_ids=[(4, return_picking.id)],
                                                      button_validate_picking_ids=[return_picking.id]).create({}).process()
        return True
