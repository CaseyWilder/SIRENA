...
class RemovalOrder(models.Model):
    ...
    removal_order_picking_ids = fields.One2many("stock.picking", 'removal_order_id',
                                                string="Removal Pickings",
                                                help="This fields relocates removal order picking "
                                                     "ids.")
    ...
    def write(self, vals):
        ...
        if 'removal_disposition' in vals:
            if vals.get('removal_disposition') == 'Disposal':
                self.removal_order_lines_ids.write(
                    {'removal_disposition': 'Disposal', 'sellable_quantity': 0.0})
            else:
    ...
    def import_product_for_removal_order(self):
        ...
        """
        import_obj = self.env['import.product.removal.order.wizard'].create( \
    ...
    def check_validate_fields(self):
        """
        This Method check validate fields and raise UserError.Check all type of validation like check removal order line, check removal order config ids etc..
        :return: This Method return Boolean(True/False).
        """
        ...
        if self.removal_order_picking_ids.filtered(lambda picking: picking.state != 'cancel'):
            raise UserError(_(
                "Pickings already exist for removal order first you should cancel all pickings"))
        return True
    ...
    def disposal_order_pickings(self):
        ...
        if not config:
            message = "Removal Order configuration missing for disposition %s" % (
                self.removal_disposition)
        ...
        for removal_line in self.removal_order_lines_ids:
            ...
            if sellable_quantity > 0.0:
                if not sellable_picking:
                    pick_vals = self.create_picking_vals(picking_type_id,
                                                         sellable_source_location_id,
                                                         dest_location_id)
    ...
    def removal_order_procurements(self):
        ...
        sellable_rule = config.sellable_route_id.rule_ids.filtered(
            lambda l: l.location_src_id.usage == 'transit')
        for removal_line in self.removal_order_lines_ids:
            ...
            if unsellable_quantity > 0.0:
                ...
                location_id = unsellable_rule.location_id
                self.run_procurement_group(proc_group_obj, product_id, qty, product_uom,
                                           location_id, amz_product.name, self.name,
            ...
            if sellable_quantity > 0.0:
                ...
                location_id = sellable_rule.location_id
                self.run_procurement_group(proc_group_obj, product_id, qty, product_uom,
                                           location_id, amz_product.name, self.name,
                                           removal_order_instance, datas)

        pickings = picking_obj.search(
            [('group_id', 'in', [sell_proc_group.id, unsell_proc_group.id])])
    ...
    def create_procurement_group(self):
        ...
        """
        proc_group_obj = self.env['procurement.group']
        return proc_group_obj.create({'removal_order_id': self.id})
    ...
    def create(self, vals):
        ...
        return super(RemovalOrder, self).create(vals)
