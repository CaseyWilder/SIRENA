...
class AmazonSellerEpt(models.Model):
    ...
    developer_name = fields.Char(help="Developer name for use Amazon MWS")
    ...
    def auto_import_fba_shipment_status_ept(self, args={}):
        # updated By: Dhaval Sanghani [12-Jun-2020]
        ...
        if seller_id:
            ...
            if amazon_inbound_shipments:
                job_id = self.env['common.log.book.ept'].create({'module': 'amazon_ept',
                                                                 'type': 'import'})
                status_result = inbound_shipment_obj.check_status_ept(amazon_inbound_shipments,
                                                                      seller, job_id)
                max_range = 0
    ...
    def update_user_group_deactive_seller(self, amazon_selling):
        ...
        if amazon_selling == 'FBA':
            ...
            if other_seller and seller_company:
                ...
                seller_company = other_seller.filtered(lambda x: x.company_id.id in self.env.user.company_ids.ids)
                other_fba_seller_company = other_fba_seller.filtered(
            ...
            if other_seller and seller_company:
                ...
                seller_company = other_seller.filtered(lambda x: x.company_id.id in self.env.user.company_ids.ids)
                other_seller_fbm_company = other_fbm_seller.filtered(
                    lambda x: x.company_id.id in self.env.user.company_ids.ids)
    ...
    def auto_create_stock_adjustment_configuration(self):
        ...
        for group_id in reason_group_ids:
            ...
            elif group_id.id == self.env.ref('amazon_ept.amazon_unrecoverable_inventory_ept').id:
                prepare_config_vals.append(
            ...
            elif group_id.id == self.env.ref('amazon_ept.amazon_software_corrections_ept').id:
                prepare_config_vals.append(
                    {'seller_id': self.id, 'group_id': group_id.id, 'location_id': inventory_location.id})
            elif group_id.id == self.env.ref('amazon_ept.amazon_transferring_ownership_ept').id:
                prepare_config_vals.append(
                    {'seller_id': self.id, 'group_id': group_id.id, 'location_id': customer_location.id})
            elif group_id.id == self.env.ref('amazon_ept.amazon_catalogue_management_ept').id:
                prepare_config_vals.append(
                    {'seller_id': self.id, 'group_id': group_id.id, 'location_id': inventory_location.id})
