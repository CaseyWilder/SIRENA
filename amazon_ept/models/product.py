...
class AmazonProductEpt(models.Model):
    ...
    def search_product(self, seller_sku):
        ...
        if self.env.ref('product.product_comp_rule').active:
            ...
            if len(product) > 1:
                raise UserError(_('Duplicate Product Found For Sku (%s)' % seller_sku))
            if not product:
                product = product_obj.search( \
                    ['|', ('company_id', '=', False), ('company_id', '=', company_id),
    ...
    condition = fields.Selection([('New', 'New'),
                                  ...
                                  ('Club', 'Club')], default='New', copy=False)
    ...
    item_height = fields.Float(help="Height of the item dimension", digits="Stock Height")
    item_length = fields.Float(help="Length of the item dimension", digits="Stock Height")
    item_width = fields.Float(help="Width of the item dimension", digits="Stock Height")

    package_dimensions_uom = fields.Selection(
        [('CM', 'CM'), ('FT', 'FT'), ('M', 'M'), ('IN', 'IN'), ('MM', 'MM')],
        string="Package Dimension", default='CM')
    package_height = fields.Float(help="Height of the package dimension", digits="Stock Height")
    package_length = fields.Float(help="Length of the package dimension", digits="Stock Height")
    package_width = fields.Float(help="Width of the package dimension", digits="Stock Height")
    ...
    def export_product_amazon(self, instance):
        ...
        kwargs = self.get_amazon_product_request_data_ept(instance, data,
                                                          'export_product_amazon_v13')
    ...
    def process_export_product_amazon_result(self, instance, data, results):
        ...
        if results.get('FeedSubmissionInfo', {}).get('FeedSubmissionId', {}).get('value', False):
            last_feed_submission_id = results.get(
                'FeedSubmissionInfo', {}).get('FeedSubmissionId', {}).get('value', False)
    ...
    def get_discontinue_date(self, product):
        ...
        discontinue_date = product.discontinue_date and datetime.strftime( \
            product.discontinue_date, "%Y-%m-%d %H:%M:%S") or False
    ...
    def get_release_date(self, product):
        ...
        release_date = product.release_date and datetime.strftime( \
            product.release_date, "%Y-%m-%d %H:%M:%S") or False
    ...
    def export_stock_levels(self, instance):
        ...
        self.process_export_stock_message_info_ept( \
            instance, product_ids.ids, amazon_products, warehouse_ids)
        return True
    ...
    def export_amazon_stock_levels_operation(self, instance):
        ...
        warehouse_ids = instance.warehouse_id.ids
        warehouse_ids += instance.stock_update_warehouse_ids.ids if \
            instance.stock_update_warehouse_ids else []
        warehouse_ids = list(set(warehouse_ids))
        ...
        self.process_export_stock_message_info_ept( \
            instance, product_ids.ids, amazon_products.ids, warehouse_ids)
        return True
    ...
    def prepare_export_stock_level_dict_operation(self, amazon_product, instance, actual_stock,
        ...
        </Message>""" % (message_id, seller_sku, stock, fullfillment_latency)
