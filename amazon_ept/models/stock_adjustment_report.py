...
class StockAdjustmentReportHistory(models.Model):
    ...
    def auto_import_stock_adjustment_report(self, args={}):
        ...
        if seller_id:
            ...
            inv_report = self.create({'seller_id': seller_id,
    ...
    def process_stock_adjustment_report(self):
        ...
        job = amazon_process_job_log_obj.search([('model_id', '=', self.env['ir.model']._get(\
                'amazon.stock.adjustment.report.history').id), ('res_id', '=', self.id)])
        if not job:
    ...
    def _amz_get_adjustment_date(date):
        ...
        return adjustment_date # datetime.fromtimestamp(adjustment_date)
