...
class FbmSaleOrderReportEpt(models.Model):
    ...
    def process_prepare_unshipped_order_list_ept(self, unshipped_order_list,
                                                 business_prime_dict, log_book, fbm_order_dict):
                ...
                if order_status == 'Unshipped':
                    if not amazon_order_ref in unshipped_order_list:
                        ...
                        if is_business_order or is_prime_order:
                            ...
                            fbm_order_dict.update({amazon_order_ref: {
                                'vat-number': vat,
                                'vat-country': vat_country_code}})
        ...
        if not delivery:
            ...
            if not delivery:
                delivery = partner_obj.with_context(tracking_disable=True).create({
                    'name': shipping_address_name, 'type': 'delivery', 'parent_id': partner.id, **new_partner_vals})
        if is_invalid_vat:
