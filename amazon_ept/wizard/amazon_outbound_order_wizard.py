...
class AmazonOutboundOrderWizard(models.TransientModel):
    ...
    @staticmethod
    def validate_amz_outbound_orders_required_fields(order, is_auto_process):
        """
        ...
        if not order.amz_shipment_service_level_category:
            ...
            if is_auto_process:
                ...
                raise UserError(_("Field FBA Shipping Speed is required for order %s" % (order.name)))
        if not order.note:
            skip_order = True
            if is_auto_process:
                order.message_post(body=_("Field Displayable Order Comment is required for order %s" % (order.name)))
            else:
                raise UserError(_("Field Displayable Order Comment is required for order %s" % (order.name)))
        if not order.amz_fulfillment_action:
            ...
            if is_auto_process:
                ...
                raise UserError(_("Field Order Fulfillment Action is required for order %s" % (order.name)))
        if not order.amz_displayable_date_time:
            ...
            if is_auto_process:
                ...
                raise UserError(_("Field Displayable Order Date Time is required for order %s" % (order.name)))
        if not order.amz_fulfillment_policy:
            skip_order = True
            if is_auto_process:
                order.message_post(body=_("Field Fulfillment Policy is required for order %s" % (order.name)))
            else:
                raise UserError(_("Field Fulfillment Policy is required for order %s" % (order.name)))
        return skip_order
