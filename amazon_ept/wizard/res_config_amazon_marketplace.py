...
class AmazonMarketplaceConfig(models.TransientModel):
    ...
    amazon_selling = fields.Selection([('FBA', 'FBA'),
                                       ('FBM', 'FBM'),
                                       ('Both', 'FBA & FBM')])
    other_marketplace_ids = fields.Many2many('amazon.marketplace.ept',
    ...
    def onchange_amazon_us_program(self):
        """
        Use: apply domain for the NARF program if select the program:
        Added on: 07th Dec, 2020
        :return: domain
        """
        domain = dict()
        if self.amz_fba_us_program:
            marketplace_ids = self.seller_id.marketplace_ids.filtered(lambda x: x.market_place_id in ['ATVPDKIKX0DER', 'A1AM78C64UM0Y8', 'A2EUQ1WTGCTBG2'])
    ...
    def _onchange_amazon_program(self):
        ...
        if self.amazon_program == 'cep':
            ...
            cep_marketplaces = marketplace_id.filtered(lambda marketplace: marketplace.market_place_id != 'A1F83G8C2ARO7P')
        ...
        if self.amazon_program == 'mci':
            ...
            inv_store_country = self.env['res.country'].search([('code', 'in', ['DE', 'IT', 'FR', 'ES', 'CZ', 'PL']),
        ...
        if self.amazon_program == 'efn':
            ...
            efn_marketplaces = marketplace_id.filtered(lambda marketplace: marketplace.market_place_id != 'A1F83G8C2ARO7P')
    ...
    def get_fba_warehouse(self, view_location_id, lot_stock_id, marketplace, unsellable_location, country=False):
        ...
        if marketplace:
            ...
            code = country.code
        code = "%s%s" % ('com' if self._context.get('amazon_program', False) and self._context.get('amazon_program') == 'narf' else code, self.seller_id.id)
        fba_wh = self.search_fba_warehouse(code)
        if fba_wh:
            return fba_wh
        partner_vals = {'name': code, 'country_id': country.id if country else False}
        partner = partner_obj.create(partner_vals)
        wh_name = self._context.get('warehouse_name') if self._context.get('warehouse_name', False) else 'FBA %s(%s)' % (
    ...
    def create_unsellable_location(self):
        ...
        if not unsellable_location or self._context.get('unsellable_name_ept', False):
            location_name = self._context.get('unsellable_name_ept') if self._context.get('unsellable_name_ept', False) else self.seller_id.name
            unsellable_vals = {
        ...
        if self.amazon_selling != 'FBM':
            ...
            if not self.seller_id.is_european_region:
                ...
                for marketplace in self.marketplace_ids:
                    ...
                    if self.amz_fba_us_program == 'narf':
                        us_country_id = self.env['res.country'].search([('code', '=', 'US')])
                        fba_warehouse_id = self.seller_id.mapped('amz_warehouse_ids').filtered(lambda r:
                                                                                               r.is_fba_warehouse == True and
                                                                                               r.partner_id.country_id.id == us_country_id.id)
            ...
            if self.amazon_program == 'pan_eu':
                ...
                for marketplace in marketplace_ids:
                    ...
                    if instance_exist:
                        if marketplace.market_place_id not in ['A1805IZSGTT6HS', 'A2NODRKZP88ZB9']:
                ...
                if other_eu_vals:
                    ...
                    if de_warehouse_id:
                        for marketplace_vals in other_eu_vals:
                            marketplace_vals.update({'fba_warehouse_id': de_warehouse_id.id})
                            amazon_instance_obj.create(marketplace_vals)
                ...
                for marketplace in self.mci_marketplace_ids:
                    ...
                    if marketplace.market_place_id in ['A1805IZSGTT6HS', 'A2NODRKZP88ZB9']:
                        ...
                        if not de_fba_wh:
                            inv_store_wh = self.env['res.country'].search([('code', '=', 'DE')])
                    ...
                    if marketplace.market_place_id in ['A1F83G8C2ARO7P']:
                        ...
                        if not gb_fba_wh:
                            inv_store_wh_gb = self.env['res.country'].search([('code', '=', 'GB')])
    ...
    def create_amazon_marketplace(self):
        ...
        if not self.amazon_program:
                ...
                if self.amazon_selling != 'FBM':
                    ...
                    if self.amz_fba_us_program == 'narf':
                        ...
                        fba_warehouse_id = self.seller_id.mapped('amz_warehouse_ids').filtered(
                            lambda r:r.is_fba_warehouse == True and r.partner_id.country_id.id == us_country_id.id)
                        if not fba_warehouse_id:
            ...
            for marketplace in self.pan_eu_marketplace_ids:
                instance_exist = self.seller_id.instance_ids.filtered(lambda x: x.marketplace_id.id == marketplace.id)
                if instance_exist:
                    continue
                vals = self.prepare_amazon_marketplace_vals(marketplace, warehouse_id)
                # Skip Creating FBA Warehouse for NL and SE Marketplace
                if marketplace.market_place_id not in ['A1805IZSGTT6HS', 'A2NODRKZP88ZB9']:
                    fba_warehouse_id = self.get_fba_warehouse(view_location_id, lot_stock_id,
                                                              marketplace, unsellable_location,
                                                              marketplace.country_id)
                    ...
                    # Set Germany instance warehouse as FBA warehouse for NL and SE Instance
                    other_eu_vals.append(vals)
                    continue
                amazon_instance_obj.create(vals)
            if other_eu_vals:
            ...
            if self.other_pan_europe_country:
                for country in self.other_pan_europe_country:
                    if self.seller_id.amz_warehouse_ids.filtered(lambda l: l.partner_id.country_id.id == country.id):
                        continue
                    self.get_fba_warehouse(view_location_id, lot_stock_id, False, unsellable_location, country)
            if self.other_marketplace_ids:
                for other_marketplace in self.other_marketplace_ids:
            ...
            for store_inv in self.store_inv_wh_cep.filtered(lambda r: r.code in ('CZ', 'PL')):
                fba_warehouse = self.seller_id.mapped('amz_warehouse_ids').filtered( \
                    lambda r: r.is_fba_warehouse and r.partner_id.country_id.id == store_inv.id)
                if not fba_warehouse:
                    unsellable_location_ept = self.with_context(unsellable_name_ept=store_inv.name).create_unsellable_location()
                    fba_warehouse = self.get_fba_warehouse(view_location_id, lot_stock_id, False,
                                                           unsellable_location_ept, store_inv)
            for marketplace in self.cep_marketplace_ids:
                instance_exist = self.seller_id.instance_ids.filtered(
                    lambda x: x.marketplace_id.id == marketplace.id)
                if instance_exist:
            ...
            for marketplace in self.mci_efn_marketplace_ids:
                ...
                if marketplace.market_place_id in ['A1F83G8C2ARO7P']:
                    ...
                    if not gb_fba_wh:
                        inv_store_wh_gb = self.env['res.country'].search([('code', '=', 'GB')])
                        unsellable_location_ept = self.with_context(unsellable_name_ept=marketplace.name).create_unsellable_location()
                        gb_fba_wh = self.get_fba_warehouse(view_location_id, lot_stock_id, False,
                                                           unsellable_location_ept, inv_store_wh_gb)
                    wh_for_mkt = gb_fba_wh if gb_fba_wh else wh_for_mkt
