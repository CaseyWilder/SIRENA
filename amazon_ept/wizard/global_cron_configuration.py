...
class GlobalCronConfiguration(models.TransientModel):
    ...
    amz_auto_send_refund_interval_number = fields.Integer('Auto Send Refund Interval Number',
                                                          help="Repeat every x.")
    amz_auto_send_refund_process_interval_type = fields.Selection(
        [('hours', 'Hours'), ('days', 'Days')],
    ...
    def update_amz_invoice_cron_field(self, amz_seller):
        """
        This method will set the values based on cron config to send invoice to amazon.
        :param amz_seller : seller record.
        """
        try:
        ...
        if amz_invoice_cron_exist:
            self.amz_auto_send_invoice = \
                amz_invoice_cron_exist.active or False
            self.amz_auto_send_invoice_interval_number = \
                amz_invoice_cron_exist.interval_number or False
            self.amz_auto_send_invoice_process_interval_type = \
                amz_invoice_cron_exist.interval_type or False
            self.amz_auto_send_invoice_next_execution = \
                amz_invoice_cron_exist.nextcall or False
            self.amz_auto_send_invoice_user_id = \
                amz_invoice_cron_exist.user_id.id or False

    def update_amz_refund_via_cron_field(self, amz_seller):
        """
        This method will set the values based on cron config to send refund invoice to amazon.
        :param amz_seller : seller record.
        """
        try:
        ...
        if amz_refund_cron_exist:
            ...
            self.amz_auto_send_refund_interval_number = \
                amz_refund_cron_exist.interval_number or False
            self.amz_auto_send_refund_process_interval_type = \
                amz_refund_cron_exist.interval_type or False
    ...
    def setup_amz_settlement_report_create_cron(self, seller):
        ...
        if self.amz_settlement_report_auto_create:
            ...
            if cron_exist:
                ...
                name = 'FBA&FBM-' + seller.name + ' : Import Settlement Report'
                vals.update({'name': name})
                new_cron = cron_exist.copy(default=vals)
                self.env['ir.model.data'].create({'module': 'amazon_ept',
    ...
    def setup_amz_settlement_report_process_cron(self, seller):
        ...
        if self.amz_settlement_report_auto_create:
            ...
            if cron_exist:
                ...
                name = 'FBA&FBM-' + seller.name + ' : Process Settlement Report'
                vals.update({'name': name})
                new_cron = cron_exist.copy(default=vals)
                self.env['ir.model.data'].create({'module': 'amazon_ept',
    ...
    def send_amz_invoice_via_email_seller_wise(self, seller):
        """
        This method will active the cron to send invoices via email.
        param amazon_seller : seller record.
        """
        if self.amz_auto_send_invoice:
            ...
            if cron_exist:
                ...
                name = 'FBA&FBM-' + seller.name + ' : Invoice Send By Email'
                vals.update({'name': name})
                new_cron = cron_exist.copy(default=vals)
                self.env['ir.model.data'].create({'module': 'amazon_ept',
    ...
    def send_amz_refund_via_email_seller_wise(self, seller):
        """
        This method will active the cron to send refund invoice via email.
        param amazon_seller : seller record.
        """
        if self.amz_auto_send_refund:
            ...
            if cron_exist:
                ...
                name = 'FBA&FBM-' + seller.name + ' : refund Send By Email'
                vals.update({'name': name})
                new_cron = cron_exist.copy(default=vals)
                self.env['ir.model.data'].create({'module': 'amazon_ept',
