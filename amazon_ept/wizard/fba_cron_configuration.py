...
class FbaCronConfiguration(models.TransientModel):
    ...
    fba_stock_adj_report_user_id = fields.Many2one("res.users",
                                                   string="Import FBA Live Stock Adjustment User")

    # inbound cron
    amz_auto_import_inbound_shipment_status = fields.Boolean(\
    ...
    amz_shipment_status_import_user_id = fields.Many2one("res.users",
    ...
    def update_amz_inbound_shipment_status(self, amz_seller):
        ...
        """
        try:
            amz_inbound_status_cron = amz_seller and self.env.ref(\
                'amazon_ept.ir_cron_inbound_shipment_check_status_%d' % amz_seller.id)
        except:
            amz_inbound_status_cron = False
    ...
    def update_amz_shipment_report_cron_field(self, amz_seller):
        ...
        """
        try:
            amz_check_shipment_report_cron_exist = amz_seller and self.env.ref(\
                'amazon_ept.ir_cron_import_amazon_fba_shipment_report_seller_%d' % (amz_seller.id))
        except:
            amz_check_shipment_report_cron_exist = False
    ...
    def update_amz_fba_live_report_cron_field(self, amz_seller):
        ...
        try:
            amz_check_live_stock_report_process_cron_exist = amz_seller and self.env.ref(\
                'amazon_ept.ir_cron_import_stock_from_amazon_fba_live_report_seller_%d' % (\
                    amz_seller.id))
        except:
    ...
    def save_cron_configuration(self):
        ...
        vals[
            'amz_auto_import_inbound_shipment_status'] = \
            self.amz_auto_import_inbound_shipment_status or False
    ...
    def setup_removal_order_report_create_cron(self, seller):
        ...
        if self.auto_create_removal_order_report and self.amazon_selling not in ['FBM']:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_create_fba_removal_order_report_seller_%d' % (
    ...
    def setup_removal_order_report_process_cron(self, seller):
        ...
        if self.auto_create_removal_order_report and self.amazon_selling not in ['FBM']:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_process_fba_removal_order_report_seller_%d' % ( \
                    seller.id), raise_if_not_found=False)
            process_next_execution = self.fba_removal_order_next_execution + relativedelta( \
                minutes=10)
            vals = {
                'active': True,
                'interval_number': self.fba_removal_order_interval_number,
    ...
    def setup_amz_seller_live_stock_cron(self, amazon_seller, auto_import, interval_type,
                                         interval_number,
                                         next_call, cron_user, prefix, cron_xml_ids,
                                         module='amazon_ept'):
        """
        ...
        for cron_xml_id in cron_xml_ids:
            if auto_import and self.amazon_selling not in ['FBM']:
                ...
                if cron_xml_id == cron_import_xml_id and ((
                    ...
                    for instnace in amazon_seller.instance_ids:
                        ...
                        vals = {
                            ...
                            'code': "model.auto_import_amazon_fba_live_stock_report({'seller_id':%d, 'instance_id': %d})"% (amazon_seller.id, instnace.id),
                        ...
                        if cron_exist:
                            ...
                            name = prefix + '-' + amazon_seller.name + ' : ' + cron_name + ' : ' + str(
                                instnace.name) if prefix else amazon_seller.name + ' : ' + cron_name + ' : ' +str(
                                instnace.name)
                            vals.update({'name': name})
                            new_cron = import_return_cron.copy(default=vals)
                            self.env['ir.model.data'].create({'module': module,
                                                              'name': cron_xml_id + '_seller_%d_instance_%d' % (
                    ...
                    if cron_xml_id != 'ir_cron_process_fba_live_stock_report' and amazon_seller.amazon_program in ['MCI', 'MCI+EFN']:
                        self.setup_amz_uk_instance_live_stock_cron(amazon_seller, interval_type, interval_number,
                                                                   next_call, cron_user, prefix)
            else:
                cron_exist = self.env.ref(
                ...
                uk_cron_exist = self.env.ref(module + '.' + 'ir_cron_import_stock_from_amazon_uk_fba_live_report_uk_seller_%d' % (
    ...
    def setup_amz_uk_instance_live_stock_cron(self, amazon_seller, interval_type, interval_number,
        ...
        if uk_insatance_id:
            ...
            if cron_exist:
                ...
                name = prefix and prefix + '-' + amazon_seller.name + ' UK ' + ' : ' + \
                       cron_name or amazon_seller.name + ' : ' + cron_name
                vals.update({'name': name})
                new_cron = ir_cron_obj.create(vals)
                self.env['ir.model.data'].create({'module': module,
                                                  'name': cron_xml_id + '_uk_seller_%d' % (amazon_seller.id),
                                                  'model': 'ir.cron',
                                                  'res_id': new_cron.id,
                                                  'noupdate': True})
        return True
    ...
    def setup_inbound_shipment_status_cron(self, seller):
        ...
        if self.amz_auto_import_inbound_shipment_status:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_inbound_shipment_check_status_%d' % (
                    seller.id), raise_if_not_found=False)
            vals = {
                'active': True,
            ...
            if cron_exist:
                ...
                self.env['ir.model.data'].create({'module': 'amazon_ept',
                                                  'name': 'ir_cron_inbound_shipment_check_status_%d' % (
                                                      seller.id),
                                                  'model': 'ir.cron',
                                                  'res_id': new_cron.id,
                                                  'noupdate': True
            ...
            cron_exist = self.env.ref('amazon_ept.ir_cron_inbound_shipment_check_status_%d' % (
                seller.id), raise_if_not_found=False)
            if cron_exist:
                cron_exist.write({'active': False})
        return True
    ...
    def setup_stock_adjustment_report_create_cron(self, seller):
        ...
        if self.auto_create_fba_stock_adj_report:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_create_fba_stock_adjustment_report_seller_%d' % (
                    seller.id), raise_if_not_found=False)
            vals = {
                'active': True,
            ...
            if cron_exist:
                ...
                inv_report_cron = self.env.ref(
                    'amazon_ept.ir_cron_create_fba_stock_adjustment_report',
                    raise_if_not_found=False)
                if not inv_report_cron:
                    raise UserError(_(
                        'Core settings of Amazon are deleted, please upgrade Amazon module to back this settings.'))
        ...
        if self.auto_create_fba_stock_adj_report:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_process_fba_stock_adjustment_report_seller_%d' % (
                    seller.id), raise_if_not_found=False)
            process_next_execution = self.fba_stock_adj_report_next_execution + relativedelta(
                minutes=10)
            vals = {
                'active': True,
                'interval_number': self.fba_stock_adj_report_interval_number,
                'interval_type': self.fba_stock_adj_report_interval_type,
                'nextcall': process_next_execution,
                'code': "model.auto_process_stock_adjustment_report({'seller_id':%d})" % (
                    seller.id),
                'amazon_seller_cron_id': seller.id,
                'user_id': self.fba_stock_adj_report_user_id and self.fba_stock_adj_report_user_id.id,
            }
            if cron_exist:
                ...
                inv_report_cron = self.env.ref(
                    'amazon_ept.ir_cron_process_fba_stock_adjustment_report',
                    raise_if_not_found=False)
                if not inv_report_cron:
                    raise UserError(_(
                        'Core settings of Amazon are deleted, please upgrade Amazon module to back this settings.'))
